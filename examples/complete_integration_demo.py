...
# Clean the data
clean_df_basic, report_basic = prep_basic.clean(task="classification", target_col="label")
...
try:
    ...
    prep_llm = AutoPrepML(df, enable_llm=True, llm_provider="ollama")

    print("✓ LLM support initialized!")

    # Get AI analysis of the dataset
    print("\n📊 Getting AI analysis of dataset...")
    analysis = prep_llm.analyze_with_llm(task="classification", target_col="label")
    print("\n" + "-" * 70)
    print("AI Analysis:")
    print("-" * 70)
    print(analysis)
    ...
    print("-" * 70)
    age_suggestions = prep_llm.get_llm_suggestions(column="age", issue_type="missing")
    ...
    print("-" * 70)
    feature_ideas = prep_llm.get_feature_suggestions(task="classification", target_col="label")
    print("\nSuggested Features:")
    ...
    explanation = prep_llm.explain_step(
        "imputed_missing", {"strategy": "iterative", "method": "MICE"}
    )
    print(explanation)
...
try:
    ...
    print("\n📊 Dataset Analysis:")
    print("-" * 70)
    analysis = suggestor.analyze_dataframe(df, task="classification", target_col="label")
    print(analysis)

    # Get specific suggestions
    print("\n💡 Suggestions for handling missing salary values:")
    print("-" * 70)
    salary_suggestions = suggestor.suggest_fix(df, column="salary", issue_type="missing")
    print(salary_suggestions)

    # Feature engineering ideas
    print("\n🔧 Feature Engineering Ideas:")
    print("-" * 70)
    features = suggestor.suggest_features(df, task="classification", target_col="label")
