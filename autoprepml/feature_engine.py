...
class AutoFeatureEngine:
    ...
    def create_polynomial_features(
        ...
        new_features = pd.DataFrame(
            poly_features[:, n_original:], columns=feature_names[n_original:], index=self.df.index
        )
    ...
    def create_interactions(
        self, columns: Optional[List[str]] = None, max_interactions: int = 10
    ) -> pd.DataFrame:
    ...
    def create_ratio_features(
        self, columns: Optional[List[str]] = None, max_ratios: int = 10
    ) -> pd.DataFrame:
        ...
        if new_features:
            self._extracted_from_create_aggregation_features_36(new_features, "ratio_features")
        return self.df
    ...
    def create_binned_features(
        ...
        if encode == "ordinal":
            binned_df = pd.DataFrame(
                binned_data, columns=[f"{col}_binned" for col in columns], index=self.df.index
            )
    ...
    def select_features(
        self, method: str = "mutual_info", k: int = 10, task: str = "classification"
    ) -> pd.DataFrame:
        ...
        if method == "f_test":
            selector = (
                ...
                else SelectKBest(score_func=f_regression, k=min(k, len(numeric_cols)))
            )
    ...
    def get_feature_importance(self, task: str = "classification") -> pd.DataFrame:
        ...
        return pd.DataFrame({"feature": numeric_cols, "importance": scores}).sort_values(
