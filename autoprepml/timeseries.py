...
class TimeSeriesPrepML:
    ...
    def __init__(self, df: pd.DataFrame, timestamp_column: str, value_column: Optional[str] = None):
        ...
        if not pd.api.types.is_datetime64_any_dtype(self.df[timestamp_column]):
            try:
                ...
                raise ValueError(f"Could not convert '{timestamp_column}' to datetime: {e}") from e
    ...
    def detect_issues(self) -> Dict[str, Any]:
        ...
        issues = {
            ...
            "date_range": {"start": str(ts_col.min()), "end": str(ts_col.max())},
        }
    ...
    def remove_duplicate_timestamps(
        ...
        self.log.append(
            {
                "action": "remove_duplicate_timestamps",
                "removed": removed,
                "method": aggregate or keep,
            }
        )
        return self.df
    ...
    def fill_missing_timestamps(self, freq: str = "D") -> pd.DataFrame:
        ...
        full_range = pd.date_range(start=self.df.index.min(), end=self.df.index.max(), freq=freq)
