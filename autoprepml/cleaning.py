...
def impute_knn(df: pd.DataFrame, n_neighbors: int = 5, exclude_cols: list = None) -> pd.DataFrame:
...
def impute_iterative(
    df: pd.DataFrame, max_iter: int = 10, random_state: int = 42, exclude_cols: list = None
) -> pd.DataFrame:
...
def encode_categorical(
    df: pd.DataFrame, method: str = "label", exclude_cols: list = None
) -> pd.DataFrame:
...
def balance_classes_smote(
    ...
    try:
        smote = SMOTE(
            sampling_strategy=sampling_strategy, k_neighbors=k_neighbors, random_state=random_state
        )
