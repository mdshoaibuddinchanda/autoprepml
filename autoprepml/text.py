...
class TextPrepML:
    ...
    def detect_issues(self) -> Dict[str, Any]:
        ...
        issues = {
            ...
            "contains_urls": int(
                text_series.str.contains(r"http[s]?://|www\.", regex=True, na=False).sum()
            ),
            "contains_emails": int(
                text_series.str.contains(r"\S+@\S+", regex=True, na=False).sum()
            ),
    ...
    def remove_stopwords(
        self, stopwords: Optional[List[str]] = None, language: str = "english"
    ) -> pd.DataFrame:
    ...
    def extract_features(self) -> pd.DataFrame:
        ...
        self.df[f"{self.text_column}_special_char_count"] = text_col.str.count(
            r'[!@#$%^&*(),.?":{}|<>]'
        )
    ...
    def filter_by_length(self, min_length: int = 5, max_length: int = 5000) -> pd.DataFrame:
        ...
        self.log.append(
            {"action": "filter_by_length", "removed": removed, "min": min_length, "max": max_length}
        )
        return self.df
