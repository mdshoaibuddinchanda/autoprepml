...
class ImagePrepML:
            ...
            try:
                ...
                if img.size != self.target_size:
                    ...
                    issues["size_mismatch"].append(
                        {"path": str(img_path), "current": img.size, "target": self.target_size}
                    )
                ...
                if img.mode != expected_mode:
                    ...
                    issues["color_mode_issues"].append(
                        {"path": str(img_path), "current": img.mode, "expected": expected_mode}
                    )
                ...
                if info["file_size"] < 1024:  # Less than 1KB
                    issues["low_quality"].append(
                        {"path": str(img_path), "file_size": info["file_size"]}
                    )

                img.close()

            except (IOError, OSError, PIL.UnidentifiedImageError) as e:
                issues["corrupted"].append({"path": str(img_path), "error": str(e)})
            except Exception as e:
                issues["wrong_format"].append({"path": str(img_path), "error": str(e)})
    ...
    def _get_expected_pil_mode(self) -> str:
        ...
        mode_map = {"rgb": "RGB", "rgba": "RGBA", "grayscale": "L", "gray": "L"}
    ...
    def clean(
        ...
        for img_path in self.image_paths:
            ...
            try:
                ...
                self.log.append(
                    {"action": "processing_error", "path": str(img_path), "error": str(e)}
                )
                continue
    ...
    def _augment_images(self, images: np.ndarray, config: Dict[str, Any]) -> np.ndarray:
...
"""
    ...
    def split_dataset(
        ...
        return (self.images[train_idx], self.images[val_idx], self.images[test_idx])
