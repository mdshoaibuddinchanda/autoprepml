...
class LLMSuggestor:
    ...
    def __init__(
        ...
        self.base_url = (
            base_url or os.getenv(f"{provider_upper}_BASE_URL") or self._get_default_base_url()
        )
    ...
    def _get_default_model(self) -> str:
        ...
        defaults = {
            ...
            LLMProvider.ANTHROPIC: os.getenv(
    ...
    def suggest_fix(
        self, df: pd.DataFrame, column: Optional[str] = None, issue_type: str = "missing"
    ) -> str:
...
"""
    ...
    def analyze_dataframe(
        self, df: pd.DataFrame, task: str = "classification", target_col: Optional[str] = None
    ) -> Dict[str, Any]:
...
def explain_cleaning_step(
    action: str, details: Dict[str, Any], provider: str = "openai", api_key: Optional[str] = None
) -> str:
    """Generate natural language explanation of a cleaning step.
...
def suggest_column_rename(
    df: pd.DataFrame, column: str, provider: str = "openai", api_key: Optional[str] = None
) -> str:
...
def generate_data_documentation(
    df: pd.DataFrame, provider: str = "openai", api_key: Optional[str] = None
) -> str:
