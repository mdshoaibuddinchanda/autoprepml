...
def sample_edges():
    ...
    return pd.DataFrame({"source": [1, 1, 2, 2, 3, 4], "target": [2, 3, 3, 4, 4, 5]})
...
def test_detect_duplicate_nodes():
    ...
    nodes = pd.DataFrame({"id": [1, 2, 2, 3], "name": ["A", "B", "B2", "C"]})  # Duplicate ID 2
...
def test_detect_duplicate_edges():
    ...
    edges = pd.DataFrame({"source": [1, 1, 2], "target": [2, 2, 3]})  # Duplicate edge 1->2
...
def test_detect_self_loops():
    ...
    edges = pd.DataFrame({"source": [1, 2, 3], "target": [2, 2, 4]})  # Self-loop: 2->2
...
def test_detect_dangling_edges(sample_nodes):
    ...
    edges = pd.DataFrame(
        {"source": [1, 2, 6], "target": [2, 3, 7]}  # Node 6 doesn't exist  # Node 7 doesn't exist
    )
...
def test_validate_node_ids():
    ...
    nodes = pd.DataFrame(
        {"id": [1, 2, None, 3, 3], "name": ["A", "B", "C", "D", "E"]}  # Missing and duplicate IDs
    )
...
def test_remove_duplicate_edges():
    ...
    edges = pd.DataFrame({"source": [1, 1, 2, 3], "target": [2, 2, 3, 4]})  # Duplicate 1->2
...
def test_identify_components_disconnected():
    ...
    edges = pd.DataFrame(
        {"source": [1, 2, 4], "target": [2, 3, 5]}  # Two components: {1,2,3} and {4,5}
    )
...
def test_filter_isolated_nodes(sample_nodes):
    ...
    # Add isolated node
    nodes = pd.concat(
        [sample_nodes, pd.DataFrame({"id": [6], "name": ["Frank"]})], ignore_index=True
    )
...
def test_weighted_graph():
    ...
    edges = pd.DataFrame({"source": [1, 2, 3], "target": [2, 3, 4], "weight": [0.5, 0.8, 1.2]})
...
def test_directed_vs_undirected():
    ...
    edges = pd.DataFrame({"source": [1, 2], "target": [2, 1]})  # Bidirectional edge
