...
def test_clean_text(sample_text_df):
    """Test text cleaning"""
    prep = TextPrepML(sample_text_df, text_column="text")
    result = prep.clean_text(lowercase=True, remove_urls=True, remove_emails=True, remove_html=True)
...
def test_detect_language():
    ...
    df = pd.DataFrame(
        {"text": ["The quick brown fox is and are", "Another English sentence the of"]}
    )
    prep = TextPrepML(df, text_column="text")
...
def test_filter_by_length():
    ...
    df = pd.DataFrame({"text": ["Short", "This is a medium length text", "X" * 6000]})  # Very long
