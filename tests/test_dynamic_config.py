...
suggestor = LLMSuggestor(
    provider="google", model="gemini-2.5-pro", temperature=0.3, max_tokens=200  # Custom model!
)
...
for provider, config in providers_config.items():
    try:
        s = LLMSuggestor(provider=provider, model=config["model"], temperature=config["temp"])
...
df = pd.DataFrame({"age": [25, 30, None, 45, 50], "salary": [50000, 60000, None, 80000, 90000]})

# Use gemini-2.5-flash (fast and efficient)
suggestor_fast = LLMSuggestor(
    provider="google", model="gemini-2.5-flash", temperature=0.4, max_tokens=300
)
