...
class TestReportGeneration:
    ...
    def test_generate_report_without_analysis(self, autoeda, tmp_path):
        ...
        self._extracted_from_test_generate_report_creates_file_3(tmp_path, autoeda)

    def test_generate_report_creates_file(self, autoeda, tmp_path):
        """Test report generation creates HTML file."""
        autoeda.analyze()
        output_path = self._extracted_from_test_generate_report_creates_file_3(tmp_path, autoeda)
...
class TestJSONExport:
    ...
    def test_to_json_without_analysis(self, autoeda, tmp_path):
        ...
        self._extracted_from_test_to_json_creates_file_3(tmp_path, autoeda)

    def test_to_json_creates_file(self, autoeda, tmp_path):
        """Test JSON export creates file."""
        autoeda.analyze()
        output_path = self._extracted_from_test_to_json_creates_file_3(tmp_path, autoeda)
...
class TestEdgeCases:
    ...
    def test_dataframe_with_only_categorical(self):
        ...
        df = pd.DataFrame({"cat1": ["A", "B", "C"] * 20, "cat2": ["X", "Y", "Z"] * 20})
        ...
        assert (
            "numeric_summary" not in results["basic_stats"]
            or len(results["basic_stats"].get("numeric_summary", {})) == 0
        )
        assert len(results["categorical"]) == 2

    def test_dataframe_with_duplicates(self):
        """Test analysis with duplicate rows."""
        df = pd.DataFrame({"a": [1, 2, 3, 1, 2, 3], "b": [10, 20, 30, 10, 20, 30]})
...
class TestPerformance:
    ...
    def test_large_dataset(self):
        ...
        df = pd.DataFrame({f"col_{i}": np.random.randn(n_samples) for i in range(20)})
