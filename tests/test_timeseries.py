...
def timeseries_with_gaps():
    ...
    return pd.DataFrame({"date": pd.to_datetime(dates), "value": [10, 20, 30, 40]})
...
def test_sort_by_time():
    ...
    df = pd.DataFrame(
        {"date": pd.to_datetime(["2024-01-03", "2024-01-01", "2024-01-02"]), "value": [30, 10, 20]}
    )
...
def test_remove_duplicate_timestamps():
    ...
    df = pd.DataFrame(
        {"date": pd.to_datetime(["2024-01-01", "2024-01-01", "2024-01-02"]), "value": [10, 15, 20]}
    )
...
def test_remove_duplicate_timestamps_aggregate():
    ...
    df = pd.DataFrame(
        {"date": pd.to_datetime(["2024-01-01", "2024-01-01", "2024-01-02"]), "value": [10, 20, 30]}
    )
...
def test_add_time_features():
    ...
    df = pd.DataFrame(
        {"date": pd.to_datetime(["2024-06-15 14:30:00", "2024-12-25 09:00:00"]), "value": [10, 20]}
    )
...
def test_resample_daily_to_weekly():
    ...
    df = pd.DataFrame(
        {"date": pd.date_range("2024-01-01", periods=14, freq="D"), "value": range(14)}
    )
