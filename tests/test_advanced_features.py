...
class TestAdvancedImputation:
    ...
    def test_knn_imputation_basic(self):
        ...
        df = pd.DataFrame(
            {"A": [1, 2, np.nan, 4, 5], "B": [5, np.nan, 7, 8, 9], "C": [10, 11, 12, 13, 14]}
        )
    ...
    def test_iterative_imputation_reproducible(self):
        ...
        df = pd.DataFrame({"A": [1, 2, np.nan, 4, 5], "B": [5, np.nan, 7, 8, 9]})
...
class TestSMOTE:
    ...
    def test_smote_basic(self):
        ...
        df = pd.DataFrame(
            {"A": list(range(100)), "B": list(range(100, 200)), "label": [0] * 90 + [1] * 10}
        )
    ...
    def test_smote_sampling_strategy_minority(self):
        ...
        try:
            result = balance_classes_smote(
                df, target_col="label", sampling_strategy="minority", random_state=42
            )
    ...
    def test_smote_k_neighbors(self):
        ...
        try:
            result = balance_classes_smote(df, target_col="label", k_neighbors=3, random_state=42)
